...
def _import_model_class_from_architecture(architecture_name: str):
    """
    Dynamically import a model class by its architecture name.
    Used for models not supported by Auto classes (e.g., Gemma3ForConditionalGeneration).
    """
    try:
    ...
    raise ImportError(f"Could not import model class: {architecture_name}")
...
class BaseModelMerger(ABC):
    ...
    def get_transformers_auto_model_class(self):
        ...
        if has_remote_code:
            ...
            # Special handling for Gemma3ForConditionalGeneration
            # AutoModelForVision2Seq doesn't support Gemma3Config, so we import the class directly
            if architecture == "Gemma3ForConditionalGeneration":
                return _import_model_class_from_architecture(architecture)
